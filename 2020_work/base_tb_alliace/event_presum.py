...
EVENT_preSum_URL = 'https://pub.alimama.com/cpa/event/preSum.json'
...
def insert_into_db(event_id,result):
    ...
    try:
        ...
        uv_comp_degree = result['eventCompDegree']
        ...
        if total_amount:
            ...
            succ = db_state.get("succ")
        if not total_amount:
            sql = 'update spider.alliance_detail set uv_num=%s,alipay_num=%s,alipay_amt=%s,quality_score=%s,unfrozen_fee=%s,pay_amt=%s,uv_comp_degree=%s where event_id=%s;'
            data =(uv_num,alipay_num,alipay_amt,quality_score,unfrozen_fee,pay_amt,uv_comp_degree,event_id)
            db_state = db.execute_data(sql, data)
            succ = db_state.get("succ")
        if succ:
...
def analyzer_info(info):
    ...
    try:
        ...
        result['eventCompDegree'] = item.get('eventCompDegree')
        if result['uv_price'] and result['uv_count']:
...
def action(event_id, shop_keeper_id, cookie):
    try:
        ...
        if succ and info:
            ...
            if not result:
                print('event_presum 数据为空')
                # message =  '淘宝联盟任务详情event_preSum数据为空'
                # send_email_warning(message)
            if result:

                succ = insert_into_db(event_id,result)
                if succ:
                     update_presum_status(event_id)
                if not succ:
...
def main():
    try:
        ...
        for _ in range(10):
            succ, item_datas = select_item_id()
            if not item_datas:
                print('event_presum succ')
                break
            for item in item_datas:
                # event_id=3295
                # keeper_info = "33568916"
                # action(event_id,keeper_info,cookie)
                print(item[0])
...
if __name__ == '__main__':
    log_file = config.get("log_dir_taobao") + "presum"
    ...
    sched.add_job(main, 'cron', hour='10',minute='31,35,40',second='0',max_instances=5)
