...
EVENT_COOPINFO_URL = 'https://pub.alimama.com/cpa/event/coopInfo.json'
...
def update_task_list(event_id):
    succ = False
    try:
        sql = "update spider.alliance_task_list set coop_status =1 where event_id ={};".format(event_id)
...
def insert_into_db(result):
    ...
    try:
        for data in result:
            adzone_id = data.get('adZoneId')
            adzone_name = data.get('adZoneName')
            pid = data.get('pid')
            event_id = data.get('eventId')

            sql = "insert into spider.alliance_coopinfo (event_id,adzone_id,adzone_name,pid)value(%s,%s,%s,%s);"
            data = (event_id,adzone_id, str(adzone_name), str(pid))
            db_state = db.execute_data(sql, data)
            succ = db_state.get("succ")
        if succ:
            print('event_id {} coopinfo succ'.format(event_id))
    except Exception as e:
        logger.exception(str(e))
    return succ
...
def analyzer_info(info):
    ...
    try:
        ...
        if j_info.get('ok') != True:
            ...
            return
        item_list = j_info.get('data').get('result')
...
def action(event_id, shop_keeper_id, cookie):
    try:
        ...
        if succ and info:
            ...
            if not result:
                print('邮件报警')
                message = '淘宝联盟任务详情event_coopinfo数据为空'
...
def select_item_id():
    ...
    datas = list()
    try:
        sql = "select event_id,shop_keeper_id from spider.alliance_task_list where coop_status=0 and status>0;"
        db_stat = db.execute_data(sql)
        succ = db_stat.get("succ")
        if not succ:
            return False
...
if __name__ == '__main__':
    log_file = config.get("log_dir_taobao") + "coopinfo"
    ...
    sched.add_job(main, 'cron', hour='10',minute='31,35',second='0',max_instances=5)
